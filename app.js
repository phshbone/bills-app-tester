...
function renderFeeding(){const el=$("feedingList");const renderItem=x=>`<div class="entry"><div class="entry-top"><div><span class="status-tag status-ongoing">${esc(x.category||"Other")}</span><strong style="display:block;margin-top:6px">${esc(x.brand)}</strong>${x.amount?`<small>Amount: ${esc(x.amount)}</small>`:""}${x.schedule?`<small>When: ${esc(x.schedule)}</small>`:""}${x.note?`<small>${esc(x.note)}</small>`:""}</div><div><button class="remove-btn" onclick="editFeeding('${x.id}')">Edit</button> <button class="remove-btn" onclick="removeFeeding('${x.id}')">Delete</button></div></div></div>`;el.innerHTML=state.feedingItems.length?collapsedHistoryHtml(state.feedingItems,renderItem,"foods & treats"):"<p>No foods, treats, chews, or supplements added yet.</p>"}
...
function removeAllergy(id){const x=state.allergies.find(v=>v.id===id);if(!x)return;if(!confirm("Remove this allergy or caution from the current list? Its history will remain in Frannie Log."))return;state.careHistory.unshift({id:uid(),type:"allergy",date:todayISO(),title:"Allergy or caution removed",detail:x.text});state.allergies=state.allergies.filter(v=>v.id!==id);if(editing.allergy===id)cancelAllergyEdit();persist();renderCare();renderMainLog()}
...
function removeWeight(id){const x=state.weights.find(v=>v.id===id);if(!x)return;if(!confirm("Remove this weight from the current measurements? Its history will remain in Frannie Log."))return;state.careHistory.unshift({id:uid(),type:"weight",date:todayISO(),title:"Weight entry removed",detail:[x.value,prettyDate(x.date),x.note].filter(Boolean).join(" · ")});state.weights=state.weights.filter(v=>v.id!==id);if(editing.weight===id)cancelWeightEdit();persist();renderCare();renderMainLog()}
...
function removeCareNote(id){const x=state.careNotes.find(v=>v.id===id);if(!x)return;if(!confirm("Archive this care note? It will leave the active notes but remain in Frannie Log history."))return;state.careHistory.unshift({id:uid(),type:"note",date:todayISO(),title:"Care note archived: "+x.title,detail:[prettyDate(x.date),x.note].filter(Boolean).join(" · ")});state.careNotes=state.careNotes.filter(v=>v.id!==id);if(editing.careNote===id)cancelCareNoteEdit();persist();renderCare();renderMainLog()}

function renderTreatments(){const renderItem=x=>{const s=treatmentStatus(x);return`<div class="entry"><div class="entry-top"><div><strong>${esc(x.name)}</strong><small>${esc(x.type)}${x.date?" · Given "+prettyDate(x.date):""}${x.due?" · Next "+prettyDate(x.due):""}</small>${x.note?`<small>${esc(x.note)}</small>`:""}</div><div><span class="status-tag ${s[1]}">${s[0]}</span><br><button class="remove-btn" onclick="editTreatment('${x.id}')">Edit</button> <button class="remove-btn" onclick="removeTreatment('${x.id}')">Delete</button></div></div></div>`};$("treatmentList").innerHTML=state.treatments.length?collapsedHistoryHtml(state.treatments,renderItem,"treatments"):"<p>No treatments or vaccinations added yet.</p>"}
function renderAllergies(){const renderItem=x=>`<div class="entry"><div class="entry-top"><strong>${esc(x.text)}</strong><div><button class="remove-btn" onclick="editAllergy('${x.id}')">Edit</button> <button class="remove-btn" onclick="removeAllergy('${x.id}')">Delete</button></div></div></div>`;$("allergyList").innerHTML=state.allergies.length?collapsedHistoryHtml(state.allergies,renderItem,"cautions"):"<p>No allergies or cautions added yet.</p>"}
function renderHeights(){const renderItem=x=>`<div class="entry"><div class="entry-top"><div><strong>${esc(x.value)}</strong><small>${prettyDate(x.date)}${x.note?" · "+esc(x.note):""}</small></div><div><button class="remove-btn" onclick="editHeight('${x.id}')">Edit</button> <button class="remove-btn" onclick="removeHeight('${x.id}')">Delete</button></div></div></div>`;$("heightList").innerHTML=state.heights.length?collapsedHistoryHtml(state.heights,renderItem,"heights"):"<p>No height entries yet.</p>"}
function renderWeights(){const renderItem=x=>`<div class="entry"><div class="entry-top"><div><strong>${esc(x.value)}</strong><small>${prettyDate(x.date)}${x.note?" · "+esc(x.note):""}</small></div><div><button class="remove-btn" onclick="editWeight('${x.id}')">Edit</button> <button class="remove-btn" onclick="removeWeight('${x.id}')">Delete</button></div></div></div>`;$("weightList").innerHTML=state.weights.length?collapsedHistoryHtml(state.weights,renderItem,"weights"):"<p>No weight entries yet.</p>"}
