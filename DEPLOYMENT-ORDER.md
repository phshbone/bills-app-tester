...
2. Verify the audited live contract still matches `CARE_DB` â†’ `care_records` â†’ ID `frannie`; do not copy or rename the care record.
...
7. Deploy the six frontend replacement files. Verify `Care â†’ Connection & activity â†’ Build CODEX-v3 Â· cache v35`.
